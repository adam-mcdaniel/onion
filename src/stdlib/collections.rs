...
pub fn register(ctx: &mut Context) {
    ...
    col_exports.insert(Expr::sym("range"), Expr::extern_fun(|args, ctx| {
        if args.len() < 2 { return Expr::Nil; }
    ...
    }, "range", "Generate range of integers"));

    col_exports.insert(Expr::sym("zip"), Expr::extern_fun(|args, ctx| {
        if args.len() != 2 { return Expr::Nil; }
    ...
    col_exports.insert(Expr::sym("flatten"), Expr::extern_fun(|args, ctx| {
         match eval_first(args, ctx) {
             Expr::List(v) => {
    ...
    col_exports.insert(Expr::sym("dedup"), Expr::extern_fun(|args, ctx| {
        match eval_first(args, ctx) {
            Expr::List(v) => {
                let mut res = Vec::new();
                for item in v {
    ...
    col_exports.insert(Expr::sym("find"), Expr::extern_fun(|args, ctx| {
         if args.len() != 2 { return Expr::Nil; }
         let list = crate::context::eval(args[0].clone(), ctx);
         let func = crate::context::eval(args[1].clone(), ctx);
    ...
    col_exports.insert(Expr::sym("any"), Expr::extern_fun(|args, ctx| {
         if args.len() != 2 { return Expr::Nil; }
         let list = crate::context::eval(args[0].clone(), ctx);
         let func = crate::context::eval(args[1].clone(), ctx);
    ...
    col_exports.insert(Expr::sym("all"), Expr::extern_fun(|args, ctx| {
         if args.len() != 2 { return Expr::Nil; }
         let list = crate::context::eval(args[0].clone(), ctx);
         let func = crate::context::eval(args[1].clone(), ctx);
    ...
    // Access
    col_exports.insert(Expr::sym("get"), Expr::extern_fun(|args, ctx| {
        if args.len() != 2 { return Expr::Nil; }
        ...
        match (col, key) {
            ...
            (Expr::Str(s), Expr::Int(i)) => {
                 ...
                 if idx >= 0 && idx < s.len() as i64 {
                     ...
                     s.chars().nth(idx as usize).map(|c| Expr::Str(c.to_string())).unwrap_or(Expr::Nil)
                 } else {
                     Expr::Nil
                 }
            }
    ...
    }, "get", "Get element by index (List/Str) or key (Map)"));

    col_exports.insert(Expr::sym("enumerate"), Expr::extern_fun(|args, ctx| {
        if args.len() != 1 { return Expr::Nil; }
        let col = crate::context::eval(args[0].clone(), ctx);
